...
import { AgentOptionsError, AgentPathError, resolveDetailedValue } from '../src/core/path'
...
describe('resolveDetailedValue', () => {
  ...
  it('reports a missing path without leaking a native exception', () => {
    expect(() => resolveDetailedValue({ setup: {} }, ['setup', 'missing'])).toThrowError(AgentPathError)
    try {
  ...
  it('rejects pagination over the hard limit', () => {
    expect(() => resolveDetailedValue([1], [], { limit: 201 })).toThrowError(AgentOptionsError)
    try {
  ...
  it('returns a leaf getter failure as an error value', () => {
    ...
    expect(result.value).toEqual({ $type: 'error', name: 'Error', message: 'getter failed' })
  })

  it('returns throwing array page entries as error values', () => {
    const rows: unknown[] = []
    Object.defineProperty(rows, 0, { enumerable: true, get: () => { throw new Error('row failed') } })
    rows.length = 1
  ...
  it('preserves own __proto__ keys in object pages', () => {
    ...
    const parsed = JSON.parse(JSON.stringify(result.value)) as Record<string, unknown>
