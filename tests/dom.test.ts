...
describe('createDOMLocators', () => {
  it('returns JSON locators for multiple fragment roots', () => {
    document.body.innerHTML = '<main><button id="save" class="primary"> Save now </button><button>Cancel</button></main>'
    const roots = [...document.querySelectorAll('button')]
    const locators = createDOMLocators(roots)
    expect(locators).toHaveLength(2)
    expect(locators[0]).toMatchObject({ selector: '#save', tag: 'button', id: 'save', classes: ['primary'], text: 'Save now', connected: true })
