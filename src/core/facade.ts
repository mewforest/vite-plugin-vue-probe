...
import {
  INITIAL_SERIALIZATION_DEFAULTS,
  serializeProbeValue,
} from "./serializer";

export const PROBE_API_VERSION = "0.1.0";
...
function serializeMap(
  ...
  prefix: string,
  budget: ReturnType<typeof stateBudget>,
): Record<string, ProbeValue> | undefined {
  if (!map) return undefined;
  const result: Record<string, ProbeValue> = Object.create(null) as Record<
    string,
    ProbeValue
  >;
  for (const key of Object.keys(map)) {
    try {
      result[key] = serializeProbeValue(Reflect.get(map, key), {
        ...budget,
        path: [prefix, key],
      });
    } catch (error) {
      result[key] = serializeProbeValue(error);
    }
  }
  return result;
}
...
function componentResult(
  ...
  serializeProbeValue(null, budget);
  const state: ComponentStateResult["state"] = {};
  for (const section of Object.keys(raw.state) as Array<
    keyof RawComponentState["state"]
  >) {
  ...
  return {
    ...
    state,
    ...(options.includeMetadata && raw.metadata
...
export function createProbeAPI(source: ProbeDataSource): ProbeAPI {
  ...
  const run = async <T>(
    ...
    try {
      return {
        ...
        meta: meta(source.getActiveAppId()),
      };
    } catch (error) {
      return {
        ok: false,
  ...
  return {
    ...
    getComponentTree: (options: ComponentTreeOptions = {}) =>
      runForApp(options.appId, async (id) => {
        if (
          ...
          throw new ProbeOptionsError(
            "maxDepth must be a non-negative integer or null",
          );
    ...
    getComponentState: (componentId, options: StateReadOptions = {}) =>
      runForApp(options.appId, async (id) => {
        staleCheck(id, options.expectedRevision);
        return componentResult(
          await source.getComponentState(id, componentId),
          options,
        );
      }),
    getDetailedState: (
      ...
      runForApp(target.appId, async (id) => {
        staleCheck(id, options.expectedRevision);
        const raw =
    ...
    getPiniaStores: (options = {}) =>
      runForApp(options.appId, (id) =>
        source.getPiniaStores(id, options.filter),
      ),
    getPiniaState: (storeId, options: StateReadOptions = {}) =>
      runForApp(options.appId, async (id) => {
        staleCheck(id, options.expectedRevision);
        return piniaResult(await source.getPiniaState(id, storeId), options);
      }),
    getComponentDOM: (componentId) =>
      runForApp(undefined, (id) => {
        return {
          appId: id,
          componentId,
          roots: createDOMLocators(source.getComponentRoots(id, componentId)),
        };
      }),
  };
}
