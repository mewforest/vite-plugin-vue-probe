import type { ComponentStateMetadata, StateEntryMetadata, StoreReferenceAgentValue } from '../public-types'
import type {
...
function storeReference(value: unknown, appId: string): StoreReferenceAgentValue | undefined {
  if (typeof value !== 'object' || value === null || !('_custom' in value))
    return undefined
  const custom = Reflect.get(value, '_custom') as Record<string, unknown>
  if (custom.type !== 'store')
    return undefined
  const storeId = custom.id ?? custom.value
  return typeof storeId === 'string' ? { $type: 'store-reference', storeId, appId } : undefined
}

function entryMetadata(entry: InspectorStateEntry): StateEntryMetadata | undefined {
  const reactivity: StateEntryMetadata['reactivity'] = entry.objectType === 'other' ? 'plain' : entry.objectType
  const metadata: StateEntryMetadata = {
    ...(reactivity ? { reactivity } : {}),
    ...(typeof entry.editable === 'boolean' ? { readonly: !entry.editable } : {}),
    ...(entry.meta?.type ? { propType: entry.meta.type } : {}),
    ...(typeof entry.meta?.required === 'boolean' ? { required: entry.meta.required } : {}),
  }
  return Object.keys(metadata).length ? metadata : undefined
}

export function normalizeComponentState(payload: InspectorComponentPayload, appId: string): RawComponentState {
  ...
  for (const entry of payload.state) {
    ...
    if (meta) {
      const sectionMeta = metadata[section] ??= Object.create(null) as NonNullable<ComponentStateMetadata[typeof section]>
...
export function normalizePiniaState(payload: InspectorPiniaPayload, appId: string, storeId: string): RawPiniaState {
  const state = section(payload.state)
  const getters = section(payload.getters)
  const customProperties = section(payload['custom properties'] ?? payload.customProperties)
  return {
    appId,
    storeId,
    state,
